...
def residual_to_pil(x, ref, thumb_size, gain):
    # x/ref 值域是 [-1, 1]，所以 abs diff 除以 2 以后才是 [0, 1] 量级。
...
def save_gt_residual_grid(
    ...
    左边三列是 GT、LR-up、|LR-up - GT|；右边三角区域是
    |x_pred(z_t, r, t) - GT|。
    """
    ...
    for col, r_value in enumerate(r_values):
        ...
        draw_center_text(draw, (x, 20, x + cell, header_h), f"|pred-GT| r={r_value:.1f}")
    ...
    for row, t_value in enumerate(t_values):
        ...
        canvas.paste(lr_gt_residual, (left_x0 + 2 * (cell + gap), y))
        for col, r_value in enumerate(r_values):
            image = pred_gt_residuals.get((t_value, r_value))
...
def main():
    ...
    for idx, path in enumerate(paths):
        ...
        lr_gt_residual = residual_to_pil(c[0], gt[0], args.thumb_size, args.residual_gain)
        ...
        for t_value in t_values:
            ...
            for r_value in r_values:
                ...
                pred_gt_residuals[(t_value, r_value)] = residual_to_pil(
                    pred[0], gt[0], args.thumb_size, args.residual_gain
                )
                key = f"t{t_value:.1f}_r{r_value:.1f}"
                residual_metrics[key] = {
                    "pred_mae_vs_gt": mae_pair(pred, gt),
                    "pred_psnr_vs_gt": psnr_pair(pred, gt),
                }
                all_pred_gt_mae.append(residual_metrics[key]["pred_mae_vs_gt"])

        lr_gt_mae = mae_pair(c, gt)
        all_lr_gt_mae.append(lr_gt_mae)
        out_path = out_dir / f"sample_{idx:02d}_pred_gt_residual_grid.png"
        save_gt_residual_grid(
            out_path,
            reference_images,
            lr_gt_residual,
            pred_gt_residuals,
        ...
        samples.append(
            {
                ...
                "path": str(path),
                "grid": str(out_path),
                "lr_up_mae_vs_gt": lr_gt_mae,
                "lr_up_psnr_vs_gt": psnr_pair(c, gt),
    ...
    summary = {
        ...
        "residual_definition": "absolute RGB residual: abs(image - GT), visualized as abs(diff) / 2 * residual_gain",
        "lr_up_vs_gt_mae_mean": float(np.mean(all_lr_gt_mae)),
        "all_pred_vs_gt_mae_mean": float(np.mean(all_pred_gt_mae)),
        "samples": samples,
    }
