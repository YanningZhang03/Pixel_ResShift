    ...
    return _wrap(
        {
            ...
            "output_dir": os.path.join(project_root, "outputs", "pixel_resshift_4"),
            "checkpoint_dir": os.path.join(project_root, "outputs", "pixel_resshift_4", "checkpoints"),
            "log_dir": os.path.join(project_root, "outputs", "pixel_resshift_4", "logs"),
            "image_dir": os.path.join(project_root, "outputs", "pixel_resshift_4", "images"),
            "seed": 42,
            "model": {
                ...
                "in_channels": 3,
                "cond_channels": 3,
                # SR 任务不需要 ImageNet 的类别条件，默认退化成单一虚拟类别。
                "num_classes": 1,
                "eval_mode": False,
            },
            "meanflow": {
...
def load_config(path=None):
    if path is None:
        path = os.environ.get("PIXEL_RESSHIFT_3_CONFIG")
    if path is None:
        return Config()
    import yaml
