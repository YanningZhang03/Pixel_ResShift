...
class PixelResShiftMeanFlow(nn.Module):
    ...
    def __init__(self, model, config):
        ...
        self.pixel_l1_lambda = float(getattr(mf_cfg, "pixel_l1_lambda", 0.0))
        self.lr_consistency_lambda = float(getattr(mf_cfg, "lr_consistency_lambda", 0.0))
        self.lr_consistency_mode = str(getattr(mf_cfg, "lr_consistency_mode", "bicubic"))
        self.use_label_condition = bool(getattr(mf_cfg, "use_label_condition", False))
    ...
    def _build_class_to_idx(self):
        """根据 ImageNet 目录名构造 wnid -> label 映射，规则与 torchvision ImageFolder 一致。"""

        if self._class_to_idx is not None:
            return self._class_to_idx

        roots = []
        train_cfg = getattr(self.config.data, "train", None)
        if train_cfg is not None:
            params = getattr(train_cfg, "params", {})
            dir_paths = getattr(params, "dir_paths", None)
            if dir_paths:
                roots.extend([str(path) for path in dir_paths])
    ...
    def _labels_from_paths(self, paths, device):
        class_to_idx = self._build_class_to_idx()
        labels = []
        for path in paths:
            from pathlib import Path

            wnid = Path(str(path)).parent.name
            labels.append(class_to_idx.get(wnid, 0))
        return torch.tensor(labels, dtype=torch.long, device=device)

    def _labels_from_batch(self, batch, batch_size, device):
        # 默认取消真实类别条件：保留 pMF 的 1000 类 embedding 权重，但统一喂 null class。
        # 这样模型不再从 ImageNet label 获取语义，只依赖 LR 图像条件和像素路径。
        if not self.use_label_condition:
            return torch.full((batch_size,), self.num_classes, dtype=torch.long, device=device)

        # 如果显式打开类别条件，ResShift 数据集会把 gt_path 透传过来，
        # 可以从父目录 wnid 解析真实 ImageNet 类别。
        y = batch.get("y", batch.get("label", None))
        if y is None:
            paths = batch.get("gt_path", batch.get("path", None))
            if paths is not None and self.num_classes >= 1000:
                return self._labels_from_paths(paths, device)
    ...
    def sr_anchor_losses(self, pred_x, x0, lq):
        """SR 内容锚定损失。

        pMF 主损失主要约束速度场；这里额外直接约束一步预测图：
        - L1 约束 pred_x 接近 GT，减少语义重画；
        - LR consistency 约束 pred_x 下采样后仍能解释输入 LR，让 LR 拿回结构主权。
        """

        bz = x0.shape[0]
        dev = x0.device
        pred_clamped = pred_x.clamp(-1, 1)

        if self.pixel_l1_lambda > 0:
            loss_l1 = (pred_clamped - x0).abs().mean(dim=(1, 2, 3))
        else:
            loss_l1 = torch.zeros(bz, device=dev)

        if self.lr_consistency_lambda > 0:
            pred_lq = self._resize(pred_clamped, lq.shape[-2:], mode=self.lr_consistency_mode)
            loss_lr = (pred_lq - lq).abs().mean(dim=(1, 2, 3))
        else:
            loss_lr = torch.zeros(bz, device=dev)

        return loss_l1, loss_lr
    ...
    def forward_loss(self, batch):
        ...
        loss_l1_raw, loss_lr_raw = self.sr_anchor_losses(pred_x, x0, lq)
        loss_aux = (
            self.adp_wt_fn(loss_lpips_raw) * self.lpips_lambda
            + self.adp_wt_fn(loss_convnext_raw) * self.convnext_lambda
            + loss_l1_raw * self.pixel_l1_lambda
            + loss_lr_raw * self.lr_consistency_lambda
        )
        ...
        metrics = {
            ...
            "loss_l1": float(loss_l1_raw.mean().detach()),
            "loss_lr": float(loss_lr_raw.mean().detach()),
            "noise_t": float(t.mean().detach()),
        }
        return loss, metrics
    ...
    def restore(self, lq, y=None, add_noise=True):
        ...
        if y is None or not self.use_label_condition:
            # 推理也取消类别条件，统一使用 pMF 的 null class token。
            y = torch.full((bz,), self.num_classes, dtype=torch.long, device=dev)
