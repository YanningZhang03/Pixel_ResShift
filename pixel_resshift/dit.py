...
class ImageCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        # 小门控初始化：接近 pMF_torch 原主干，同时让 LR 条件分支从第一步就能收到梯度。
        self.cross_scale = nn.Parameter(torch.full((hidden_size,), float(cross_scale_init)))
...
class ConditionalPMFDiT(_BasePMFDiT):
    ...
    def __init__(self, *args, cond_channels=3, cross_scale_init=0.0, **kwargs):
        ...
        if self.cond_channels > 0:
            ...
            cross_kwargs = dict(
                ...
                cross_scale_init=float(cross_scale_init),
            )
...
def build_model(config):
    ...
    return model_fn(
        ...
        cross_scale_init=float(getattr(model_cfg, "cross_scale_init", 0.0)),
