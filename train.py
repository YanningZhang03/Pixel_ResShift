...
def save_training_visualization(config, flow, batch, global_step, writer=None):
    ...
    save_triplet_image(image_path, gt[0], lq_up[0], pred[0])
...
def save_checkpoint(config, model, optimizer, scheduler, step):
    path = os.path.join(config.checkpoint_dir, f"pixel-resshift-4-step-{step}.pt")
    state = {
    ...
    for epoch in range(int(config.training.num_epochs)):
        ...
        for raw_batch in progress:
            ...
            if (
                ...
                if is_main_process():
                    save_training_visualization(config, raw_flow, batch, global_step, writer)
            ...
            if should_step:
                ...
                for key in ("loss", "loss_u", "loss_v", "loss_lpips", "loss_convnext"):
